...
int main(){
    ...
    std::cout << "Authentication successful. Executing ECS Stop action..." << std::endl;
    bool success = execute_ecs_action(config, token, false);  // false means STOP
    if (success) {
        std::cout << "Successfully sent ECS Stop action!" << std::endl;
    } else {
        std::cerr << "Failed to send ECS Stop action." << std::endl;
        return 1;
    }
